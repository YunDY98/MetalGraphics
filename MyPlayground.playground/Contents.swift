...
guard let device = MTLCreateSystemDefaultDevice() else{
    fatalError("Gpu is not supproted")
}
...
let mdlMesh = MDLMesh(
    ...
    segments: [10, 10],
    inwardNormals: false,
    cap: true,geometryType: .triangles,
    allocator: allocator)
...
guard let commandQueue = device.makeCommandQueue() else{
    fatalError("Could not create a commnad queue")
}
...
vertex float4 vertex_main(const VertxIn vertex_in [[stage_in]])
{
    return vertex_in.position;
}

fragment float4 fragment_main() {
    return float4(1, 0.766, 0.366, 1);
}
"""
